...
public class Delivery_Details_Activity extends BaseActivity implements  DatePickerDialog.OnDateSetListener,TimePickerDialog.OnTimeSetListener,InterfaceClass{
	...
	TextView RequiredDeliveryDate, Delivery_Time_From,biltocustomernameSpinner,Delivery_Time_To,DoNotDeliverTime_From,DoNotDeliverTime_To;
	...
	EditText Delivery_Location_Address;
	...
	public void fillAllDataOfDeliverySectionToView()
	{
		...
		if(helperDb.checkStoreOrderBillAddressDetails(storeID, OrderPDAID)==1)
		{
			...
			String otherBillCustAddress=strPreFilledStoreOrderBillAddressDetails.split(Pattern.quote("$"))[3];
					 ...
					 for(Map.Entry<String, String> entry:hmapStoreBillTogAddresse.entrySet())
						{
						 ...
						 String keyHashmap=entry.getKey().toString().trim();
							...
							if(currentBillToAddress.equals(ValueHashMap)){
								...
								if(hmapStoreAddress.get(hmapStoreBillTogAddresse.get(keyHashmap)).equals("Others")){
									Billing_AddressText.setText(hmapStoreAddress.get(hmapStoreBillTogAddresse.get(keyHashmap)));
									Billing_AddressText.setVisibility(View.VISIBLE);
									Billing_AddressText.setEnabled(true);

											if(!otherBillCustAddress.equals("0")){
												Billing_AddressText.setText(otherBillCustAddress);
											}


									break;
								}
								else if(hmapStoreAddress.get(hmapStoreBillTogAddresse.get(keyHashmap)).equals("Select")){
									Billing_AddressText.setText(hmapStoreAddress.get(hmapStoreBillTogAddresse.get(keyHashmap)));
									Billing_AddressText.setVisibility(View.GONE);
									break;
								}
			...
			String otherAddress=strPreFilledStoreOrderBillAddressDetails.split(Pattern.quote("$"))[2];
					 ...
					 for(Map.Entry<String, String> entry:hmapStoreShippToAddress.entrySet())
						{
						 ...
						 String keyHashmap=entry.getKey().toString().trim();
							...
							if(string_Delivery_Location_Spinner.equals(ValueHashMap)){
								...
								if(hmapStoreAddress.get(hmapStoreShippToAddress.get(keyHashmap)).equals("Others")){
									Delivery_Location_Address.setText(hmapStoreAddress.get(hmapStoreShippToAddress.get(keyHashmap)));
									if(!otherAddress.equals("0")){
										Delivery_Location_Address.setText(otherAddress);
									}

									Delivery_Location_Address.setVisibility(View.VISIBLE);

									Delivery_Location_Address.setEnabled(true);
									break;
								}
								else {
									Delivery_Location_Address.setText(hmapStoreAddress.get(hmapStoreShippToAddress.get(keyHashmap)));
			...
			//	if(validateDelivery_Location_Spinner())
				{
					...
					alertDialog.setPositiveButton("Yes",
							new DialogInterface.OnClickListener() {
								...
								{
									   dialog.dismiss();
									   Intent fireBackDetPg=new Intent(Delivery_Details_Activity.this,DistributorSelectionForSKU.class);
									...
									fireBackDetPg.putExtra("ComeFrom", ComeFrom);
	...
	public void initializeDropdown() {
		Delivery_Location_Address=(EditText) findViewById(R.id.Delivery_Location_Address);
		Billing_AddressText=(EditText) findViewById(R.id.Billing_AddressText);
			   ...
			   for(Map.Entry<String, String> entry:hmapStoreShippToAddress.entrySet())
				{
				 ...
				 String keyHashmap=entry.getKey().toString().trim();
					...
					else if(hmapStoreAddress.get(hmapStoreShippToAddress.get(keyHashmap)).equals("Select")){
						Delivery_Location_Address.setVisibility(View.GONE);
						break;
					        ...
					        listDeliveryLocation.setOnItemClickListener(new OnItemClickListener() {
								...
								public void onItemClick(AdapterView<?> arg0, View arg1,
									...
									if(abc.equals("Select")){
										...
										else if(abc.equals("Others")){
										Delivery_Location_Spinner.setText("Others");
										Delivery_Location_Address.setText("");
										Delivery_Location_Address.setEnabled(true);
										Delivery_Location_Address.setVisibility(View.VISIBLE);
	 ...
	 public boolean validatecheckboxofDelivery() {
		...
		String fullstring="0";
			String string_when_other_select="0";

			String string_bil_cust_when_other="0";
		...
		if(!Delivery_Location_Address.getText().toString().trim().equals("")){
			string_when_other_select	=Delivery_Location_Address.getText().toString().trim();
		}
		...
		if(!biltocustomernameSpinner.getText().toString().trim().equals("Select")){
			...
			if(!Billing_AddressText.getText().toString().trim().equals("")){
				string_bil_cust_when_other	=Billing_AddressText.getText().toString().trim();
			}
		...
		helperDb.open();
		helperDb.fnsaveStoreOrderBillAddressDetails(storeID, OrderPDAID, string_biltocustomernameSpinner, string_Delivery_Location_Spinner+"^"+string_Delivery_Time_From+"^"+string_Delivery_Time_To+"^"+string_DoNotDeliverTime_From+"^"+string_DoNotDeliverTime_To+"^"+string_RequiredDeliveryDate, "1",string_when_other_select,string_bil_cust_when_other);
		helperDb.close();
	 ...
	 if(!creditLimitOfCredit.getText().toString().trim().equals("")){
		...
		helperDb.close();
		 ...
		 Intent fireBackDetPg=new Intent(Delivery_Details_Activity.this,LastVisitDetail_AllButton.class);
         fireBackDetPg.putExtra("storeID", storeID);
         fireBackDetPg.putExtra("SN", SN);
        fireBackDetPg.putExtra("imei", imei);
         fireBackDetPg.putExtra("userdate", date);
	...
	private class FullSyncDataNow extends AsyncTask<Void, Void, Void> {
		...
		public FullSyncDataNow(Delivery_Details_Activity activity)
		{
		...
		protected void onPostExecute(Void result) {
			...
			try
			{
				...
				Intent syncIntent = new Intent(Delivery_Details_Activity.this, SyncMaster.class);
				...
				/*	AlertDialog.Builder alertDialog = new AlertDialog.Builder(Delivery_Details_Activity.this);
					...
					alertDialog.setPositiveButton("Yes",new DialogInterface.OnClickListener()
					{
								public void onClick(DialogInterface dialog,int which)
								{
									...
									savingDataOfDeliverySectionTodatabase();
									saveDataToDataBase();
									FullSyncDataNow task = new FullSyncDataNow(Delivery_Details_Activity.this);
									task.execute();
	...
	public boolean validateDelivery_Location_Spinner()
	{
		...
		{
			String getDeliveryLoc=Delivery_Location_Address.getText().toString().trim();
			if(Delivery_Location_Spinner.getText().toString().trim().equals("Select"))
			{
				showAlertSingleButtonError("Please Select DeliveryLocation");
			}
			else if(Delivery_Location_Address.getText().toString().trim().equals("Others") || getDeliveryLoc.isEmpty())
			{
				showAlertSingleButtonError("Please Enter Delivery Location");
			}
			else if(biltocustomernameSpinner.getText().toString().trim().equals("Others") && Billing_AddressText.getText().toString().trim().equals(""))
			{
				showAlertSingleButtonError("Please Enter Bill to address");
			}
			else
			{
