...
//nitika
	LinkedHashMap<String,ArrayList<String>> hmapLastSummaryDynamic=new LinkedHashMap<String,ArrayList<String>>();

	ArrayList<String > arrayListLastSummary=new ArrayList<>();
	LinearLayout relLayout_for_OutstandingsNew11111,ll_dynamicData;
	...
	protected void onResume() {
		...
		dbengine.close();
		//("Sunil Tty Noti_textWithMsgServerID :"+Noti_textWithMsgServerID);
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		//nitika
		ll_dynamicData= (LinearLayout) findViewById(R.id.ll_dynamicData);
		...
		ISNewStore=dbengine.fncheckStoreIsNewOrOld(storeID);
		...
		/*if(ISNewStore==1)
		{
			Intent intents=new Intent(LastVisitDetails.this,SpecialRemarksActivity.class);
			...
			intents.putExtra("flgFromFeedBackOrLastVisit", "0");
		...
		else
		{
			if(flgfeedbackReq==1)
			{
				if(dbengine.isFeedBackDone(storeID)==false)
				{
					Intent intent=new Intent(LastVisitDetails.this,FeedBack_Activity.class);
		...
		*/    dbengine.open();
		...
		Button btn_NextSteps=	(Button) findViewById(R.id.btn_NextSteps);



		if(ISNewStore==1)
		{
			btn_NextSteps.setVisibility(View.GONE);
		}
		else
		{
			if(flgfeedbackReq==1)
		...
		//quotationBtn.setVisibility(View.GONE);
			...
			{
				//quotationBtn.setVisibility(View.GONE);
				Button quotationViewBtn=(Button) findViewById(R.id.quotationViewBtn);
				quotationViewBtn.setVisibility(View.GONE);
			}
    ...
    tbl3_dyntable_SpecialSchemeApplicable = (TableLayout) findViewById(R.id.dyntable_SpecialSchemeApplicable);
		...
		nxtP4.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0)
			{
				...
				dbengine.close();
				Intent nxtP4 = new Intent(LastVisitDetails.this,LastVisitDetail_AllButton.class);
				nxtP4.putExtra("storeID", storeID);
				nxtP4.putExtra("SN", selStoreName);
				nxtP4.putExtra("imei", imei);
				nxtP4.putExtra("userdate", date);
				...
				/*if (validate())
				{
						...
						SimpleDateFormat dfnew = new SimpleDateFormat("dd-MM-yyyy HH:mm:ss",Locale.ENGLISH);
		...
		/*Button quotationViewBtn=(Button) findViewById(R.id.quotationViewBtn);
		quotationViewBtn.setOnClickListener(new OnClickListener()
		{
			@Override
			public void onClick(View v)
			{
				Intent intent=new Intent(LastVisitDetails.this,ViewQuotation.class);
				intent.putExtra("storeID", storeID);
				intent.putExtra("SN", selStoreName);
				intent.putExtra("imei", imei);
				intent.putExtra("userdate", date);
...
//nitika
	public void summaryDataDynamic()
	{
		hmapLastSummaryDynamic=dbengine.gettblAccntLastSummDetails(storeID);

	/*	arrayListLastSummary.add("Due Amount"+"^"+"1500");
		arrayListLastSummary.add("OverDue Amount"+"^"+"200");

		hmapLastSummaryDynamic.put("Outstandings",arrayListLastSummary);

		arrayListLastSummary=new ArrayList<>();
		arrayListLastSummary.add("Trial Order/SampleOrder"+"^"+"Feedback Pending");
		arrayListLastSummary.add("Pending Steps"+"^"+"Feedback to be taken");
		arrayListLastSummary.add("Quotation"+"^"+"Approved Quotation available");
		arrayListLastSummary.add("Health Relationship"+"^"+"Green");
		arrayListLastSummary.add("Regular Order Booked"+"^"+"Yes");

		hmapLastSummaryDynamic.put("",arrayListLastSummary);
*/
...
//nitika
	public void inflateRowDynamic()
	{
		LayoutInflater inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
		for(Map.Entry<String,ArrayList<String>> entry:hmapLastSummaryDynamic.entrySet())
		{
			String sectionHeader=entry.getKey();
			ArrayList<String> list_values=entry.getValue();

			View view = inflater.inflate(R.layout.custom_header_last_visit, null);

			LinearLayout ll_rowParent= (LinearLayout) view.findViewById(R.id.ll_rowParent);
			...
			final TextView txt_header=(TextView)view. findViewById(R.id.txt_header);

			if(!sectionHeader.equals(""))
			{
				txt_header.setText(sectionHeader);
				ll_parent.setBackgroundResource(R.drawable.shadow_with_5dp);
			}
			else
			{
			...
			for(int count=0;count<list_values.size();count++)
			{
				View view_row = inflater.inflate(R.layout.custom_row_last_visit, null);

				LinearLayout ll_singleRow= (LinearLayout) view_row.findViewById(R.id.ll_singleRow);
				final TextView txt_section=(TextView)view_row. findViewById(R.id.txt_section);
				final TextView txt_value=(TextView)view_row. findViewById(R.id.txt_value);

				String header=list_values.get(count).split(Pattern.quote("^"))[0];
				String value=list_values.get(count).split(Pattern.quote("^"))[1];

				txt_section.setText(header);
				if(value.equals("Green"))
				{
					txt_value.setCompoundDrawablesWithIntrinsicBounds(R.drawable.green_circle15, 0, 0, 0);
				}
				else if(value.equals("Red"))
				{
					txt_value.setCompoundDrawablesWithIntrinsicBounds(R.drawable.redcircle_15, 0, 0, 0);
				}
				else if(value.equals("Amber"))
				{
					txt_value.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ambercircle_15, 0, 0, 0);
				}
				else
				{
				...
				if(sectionHeader.equals(""))
				{
					ll_singleRow.setBackgroundResource(R.drawable.shadow_with_5dp);
				}

				ll_rowParent.addView(view_row);
			}
			ll_dynamicData.addView(view);
		}
	}
