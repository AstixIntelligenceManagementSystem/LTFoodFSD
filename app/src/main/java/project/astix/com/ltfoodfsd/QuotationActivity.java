...
static String selStoreName,SalesQuoteId,storeID,imei,date,pickerDate;
	static int StoreCurrentStoreType;
...
public boolean onKeyDown(int keyCode, KeyEvent event) 
{
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		if(FROM.equals("FEEDBACK")){
			int QuotationMarkInFeedbackOrNot=dbengine.fncheckIfAnyProductMarkedWithQuotationInFeedBack(QuotationActivity.storeID);
			if(QuotationMarkInFeedbackOrNot==1){
				searchStringMarkInFeedback=	dbengine.fnGetProductNamesForSearching(QuotationActivity.storeID,1);
			}
		}
		...
		dbengine.close();
						...
						if(arg0==1 && ff!=null)
						{
							...
							try
							{
								...
								if(!txt_Search.equals(null))
								{
									...
									{
										if(!searchStringMarkInFeedback.trim().equals("")){
											txt_Search.setText(searchStringMarkInFeedback.trim());
											((FragmentA) ff).clearViewWhenSlide();
											Button btnGo=	(Button) ff.getView().findViewWithTag("btnGo");
											btnGo.performClick();
										}
	     ...
	     actionBar.setStackedBackgroundDrawable(new ColorDrawable(Color.parseColor("#a25611")));
	            ...
	            // app icon in action bar clicked; goto parent activity.
			     ...
			     alertDialog.setPositiveButton("Yes", new DialogInterface.OnClickListener()
			     {
			            public void onClick(DialogInterface dialog,int which) 
			            {
							...
							Intent ide=new Intent(QuotationActivity.this,LastVisitDetail_AllButton.class);
